package body Predefined_Rewriters_Membership_Test is
   ...
   begin
      if Node.Is_Null then
         ...
         begin
            return
              (if Parent.Is_Null or else Parent.Kind /= Ada_Bin_Op
               then Node
               else Context (Parent));
         end;
      end if;
   ...
   begin
      ...
      Result.Append_Vector
        (Context (Rewriter_Combine_In_Ranges_Step.Matching_Nodes (Unit)));
      ...
      Result.Append_Vector
        (Context
           (Rewriter_Combine_Not_In_Range_And_Different_Step.Matching_Nodes
              (Unit)));

      Result.Append_Vector
        (Context
           (Rewriter_Combine_Different_And_Not_In_Range_Step.Matching_Nodes
              (Unit)));

      Result.Append_Vector
        (Context (Rewriter_Combine_Not_In_Ranges_Step.Matching_Nodes (Unit)));
